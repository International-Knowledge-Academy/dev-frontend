...
import {
  ...
  MdSchool, MdVerified, MdLink, MdOpenInNew,
  MdWorkspacePremium, MdCalendarToday, MdLayers, MdSettings,
} from "react-icons/md";
...
const SectionTitle = ({ title, count }: { title: string; count?: number }) => (
  <div className="flex items-center gap-2 mt-6 mb-1 px-4 sm:px-6">
    ...
    {count != null && (
      <span className="text-xs font-semibold bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded-md">{count}</span>
    )}
  </div>
);
    ...
    <div className="max-w-5xl m-auto space-y-4">
          ...
          <div className="flex-shrink-0">
            {p?.profile_picture ? (
              <img
                ...
                alt={user.name}
                className="w-14 h-14 sm:w-20 sm:h-20 rounded-full object-cover border-2 border-slate-100 shadow-sm"
              />
            ) : (
          ...
          <div className="px-4 sm:px-6">
            ...
            {p.certifications && (
              <InfoRow
                ...
                value={<span className="whitespace-pre-wrap text-slate-600">{p.certifications}</span>}
              />
            )}
            {p.linkedin_url && (
              <InfoRow
            ...
            <>
              ...
              <div className="px-4 sm:px-6 pb-2">
                <InfoRow
                  ...
                  label="CV / Resume"
                  value={
                    <a href={p.cv} target="_blank" rel="noreferrer" className="text-navy-600 hover:underline inline-flex items-center gap-1">
                      View / Download CV <MdOpenInNew size={12} />
                    </a>
                  }
                />
              </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="px-4 sm:px-6 py-4 border-b border-slate-100 flex items-center gap-2">
          <p className="text-xs font-bold uppercase tracking-widest text-slate-400">Assigned Programs</p>
          {programCount > 0 && (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-3.5 text-slate-500 text-sm">
                        {prog.field?.name ?? <span className="text-slate-300 italic">—</span>}
                      </td>
                      <td className="px-5 py-3.5">
