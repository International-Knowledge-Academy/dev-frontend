...
import {
  ...
  Tag, Trash2, Pencil, CheckCircle, XCircle, DollarSign,
} from "lucide-react";
import useAdditionalServices from "hooks/services/useAdditionalServices";
import useDeleteAdditionalService from "hooks/services/useDeleteAdditionalService";
...
const AdditionalServicesPage = () => {
  ...
  const { services, count, loading, error, params, setParams, refetch } = useAdditionalServices();
  const { deleteService, loading: deleting } = useDeleteAdditionalService();
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Manage add-on services like hotel rooms and airport transfers"
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        ...
        <StatCard icon={DollarSign}  label="Active Services" value={services.filter((s) => s.is_active).length} accent />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100">
        <div className="p-4 sm:p-6">
          <div className="rounded-xl border border-slate-100 overflow-hidden">
            {loading ? (
              <Loading text="Fetching additional services..." />
            ) : error ? (
              ...
              <EmptyState
                ...
                description="Additional services will appear here once they are added."
              />
            ) : (
                  ...
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-50/60">
                      {[
                        ...
                        { label: "Type",         icon: <Tag     size={13} /> },
                        { label: "Price",        icon: <DollarSign size={13} /> },
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-semibold text-navy-800 truncate max-w-[180px]">
                              {service.name}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-3.5">
                          {service.service_type_name ? (
                            <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-navy-50 text-navy-700 border border-navy-100">
                              <span className="truncate max-w-[120px]">{service.service_type_name}</span>
                            </span>
                          ) : (
                        ...
                        {/* Price */}
                        <td className="px-5 py-3.5 font-semibold text-navy-800 whitespace-nowrap">
                          {service.price}{" "}
                          <span className="text-xs font-normal text-slate-400">{service.currency}</span>
                        </td>
