...
import {
  MdArrowBack, MdEdit, MdLabel, MdAttachMoney,
  MdCheckCircle, MdCancel, MdCalendarToday, MdCategory,
} from "react-icons/md";
...
const ServiceDetailPage = () => {
  ...
  const { service, loading, error } = useAdditionalService(uid);
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-slate-100 flex items-center gap-3 sm:gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-400 mt-0.5 truncate">
              {service.service_type_name && `${service.service_type_name} · `}
        ...
        <div className="px-4 sm:px-6 grid grid-cols-1 md:grid-cols-2">
          ...
          <InfoRow icon={<MdCategory   size={18} />} label="Service Type"  value={service.service_type_name || "—"} />
          <InfoRow icon={<MdAttachMoney size={18} />} label="Price"        value={`${service.price} ${service.currency}`} />
        </div>
