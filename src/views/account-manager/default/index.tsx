...
/* Pipeline blocks shown in the right panel */
const PIPELINE = [
  { key: "pending",   label: "Pending",  color: "#F59E0B", bg: "#FEF3C718" },
  { key: "approved",  label: "Approved", color: "#22C55E", bg: "#F0FDF418" },
  { key: "rejected",  label: "Rejected", color: "#EF4444", bg: "#FEF2F218" },
];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-6 flex flex-col">
            <div className="flex items-center justify-between mb-5">
              <p className="text-base font-bold text-navy-800">Pipeline</p>
              <span className="text-xs font-semibold text-slate-400 tabular-nums">{totalByStatus.toLocaleString()} total</span>
            </div>

            {/* Status blocks */}
            <div className="grid grid-cols-3 gap-2 mb-5">
              {PIPELINE.map(({ key, label, color, bg }) => {
                const count = data.registrations_by_status?.[key] ?? 0;
                return (
                  <div key={key} className="text-center p-2.5 rounded-lg" style={{ backgroundColor: bg }}>
            ...
            <div className="flex h-2 rounded-full overflow-hidden gap-px mb-4">
              {statusEntries.map(([k, v]) => (
                <div
                  ...
                  style={{
                    width: `${totalByStatus > 0 ? (v / totalByStatus) * 100 : 0}%`,
                    backgroundColor: STATUS_COLORS[k] ?? "#94A3B8",
                  }}
                />
              ))}
            </div>
                ...
                return (
                  <div key={status} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-xs text-slate-500">{STATUS_LABELS[status] ?? formatKey(status)}</span>
                    </div>
