...
test.describe("Account Manager — Services", () => {
  test.beforeEach(async ({ page }) => {
    await loginAsManager(page);
  });

  test("list page loads", async ({ page }) => {
    await page.goto("/account-manager/services");
  ...
  test("create form — submit disabled when required fields empty", async ({ page }) => {
    await page.goto("/account-manager/services/create");
  ...
  test("clicking a row opens service detail", async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/account-manager\/services\/.+/);
  });
});
