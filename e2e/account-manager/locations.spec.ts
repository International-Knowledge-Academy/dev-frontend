...
test.describe("Account Manager — Locations", () => {
  test.beforeEach(async ({ page }) => {
    await loginAsManager(page);
  });

  test("list page loads", async ({ page }) => {
    await page.goto("/account-manager/locations");
  ...
  test("create form — submit disabled when required fields empty", async ({ page }) => {
    await page.goto("/account-manager/locations/create");
  ...
  test("clicking a row opens location detail", async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/account-manager\/locations\/.+/);
  });
});
